...
class Columbia_pascal_voc(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'Columbia')
        self._year = year
        self._image_set = image_set
        self._devkit_path = 'data/Columbia'
        self._data_path = os.path.join(self._devkit_path, 'Columbia')
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        for i, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            filename = self._get_voc_results_file_template().format(cls)
            rec, prec, ap, f1, acc, auc = voc_eval(
        ...
        print(('Mean AUC = {:.4f}'.format(np.mean(aucs))))
        print('~~~~~~~~')
        print('Results:')
        for ap in aps:
            print(('{:.3f}'.format(ap)))
